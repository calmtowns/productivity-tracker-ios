...
export function useTasks() {
  ...
  useEffect(() => {
    ...
    if (lastRollover !== today) {
      let changed = false
      updated = saved.map(t => {
        if (!t.completed && isPastDate(t.date)) {
          changed = true
          return { ...t, date: today, wasRolledOver: true }
        }
        return t
      })
  ...
  const addTask = useCallback(
    (data: { title: string; description: string; date: string; priority: Priority }) => {
      const task: Task = {
        id: generateId(),
        title: data.title,
  ...
  const getTasksForDate = useCallback(
    (dateStr: string) => tasks.filter(t => t.date === dateStr),
    [tasks]
  )
  ...
  const progress =
    todayTasks.length === 0 ? 0 : todayTasks.filter(t => t.completed).length / todayTasks.length
