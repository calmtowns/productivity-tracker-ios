...
export default function TasksView({ tasks, filter, onFilterChange, onToggle, onDelete, onAdd }: TasksViewProps) {
  return (
    <div className="flex flex-col gap-5">
        ...
        {FILTERS.map(f => (
          <button
            ...
            className={cn(
              ...
              filter === f.id
                ? 'bg-white dark:bg-stone-700 text-amber-800 dark:text-amber-400 shadow-card'
      ...
      {tasks.length === 0 ? (
        <EmptyState
          ...
          title={filter === 'completed' ? 'Выполненных нет' : 'Задач пока нет'}
          subtitle={
            filter === 'completed'
              ? 'Выполненные задачи появятся здесь'
              : 'Добавь первую задачу и начни день продуктивно'
          }
          action={filter !== 'completed' ? { label: 'Добавить задачу', onClick: onAdd } : undefined}
        />
      ) : (
