...
export default function EmptyState({ icon, title, subtitle, action }: EmptyStateProps) {
  return (
    ...
    >
      <div className="w-20 h-20 rounded-3xl bg-amber-50 dark:bg-stone-800 flex items-center justify-center text-4xl mb-4 shadow-inner">
        {icon}
      </div>
      ...
      {action && (
        <button
          ...
          className="mt-5 px-6 py-3 bg-amber-800 dark:bg-amber-700 text-white text-sm font-semibold rounded-2xl active:scale-95 transition-transform shadow-card"
        >
          {action.label}
        </button>
      )}
