...
export default function NoteCard({ note, onDelete }: { note: Note; onDelete: () => void }) {
  ...
  return (
    <motion.div layout className={`group relative rounded-2xl p-4 border ${colors}`}>
      <button onClick={e => { e.stopPropagation(); onDelete() }} className="absolute top-3 right-3 p-1 rounded-full opacity-0 group-hover:opacity-100 hover:bg-black/10 dark:hover:bg-white/10 transition-all" aria-label="Удалить"><Trash2 size={13} className="text-stone-400" /></button>
      {note.title && <h3 className="text-sm font-semibold text-stone-800 dark:text-cream-50 mb-1.5 pr-5 leading-snug">{note.title}</h3>}
